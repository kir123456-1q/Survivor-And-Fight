...
本初稿 LaTeX 源文件经 \texttt{tools/count-thesis-chars.cjs} 统计，正文汉字数量应不少于两万（统计范围含各章 \texttt{chapter*.tex} 与 \texttt{frontmatter.tex}，不含参考文献 BibTeX 条目）。参考文献在 \texttt{thesis/references.bib} 中提供 \textbf{20} 条占位条目，每条含【检索关键词】，请使用中国知网、IEEE Xplore、ACM DL 等数据库检索后替换为规范著录格式，并保证正文引用不少于 15 处\cite{gregory2018game,goslin2004panda,gregory2018game,gregory2018game,gregory2018game,mckenzie2008crowd,heijstek2011empirical,munaiah2017github,briand2001maintainability,gregory2018game,helbing2000panic,briand2001maintainability,gregory2018game,gregory2018game,gregory2018game}。
...
答辩 PPT 建议 15--20 页：选题背景 2 页、需求 2 页、总体架构 2 页、核心模块 4 页、性能与测试 2 页、演示截图 3 页、总结 1 页。演示视频录制 3--5 分钟，包含跑图与技能装配。携带功能模块对照表或架构示意图打印件，便于回答“你的创新点在哪里”类问题。
...
第1章说明选题背景、创新点与论文结构。第2章从引擎语言、玩法架构、UI 架构三方面综述相关技术与选型依据。第3章给出功能/非功能需求、用例、MoSCoW 优先级及量化指标。第4章描述五层架构、ECS 调度、战斗数据流、元游戏状态机与安全可靠性设计。第5章按模块详解实现，含三个典型场景与源码索引表。第6章讨论对象池、Worker、碰撞优化、绿色计算与项目管理。第7章给出测试策略、用例、性能实验设计与缺陷记录模板。第8章总结成果、不足、社会影响与展望。附录含术语表、配表字段、需求—测试映射与缺陷记录等。

\section{工具链命令速查}

提取 PDF 模板要求：\texttt{node tools/extract-thesis-template.cjs}，输出 \texttt{docs/thesis-template-requirements.txt}。统计论文字数：\texttt{node tools/count-thesis-chars.cjs}。编译论文：在 \texttt{thesis/} 目录执行 \texttt{xelatex main.tex}、\texttt{bibtex main}、\texttt{xelatex main.tex}（两次）。同步配表：\texttt{tools/sync-config-to-bin.ps1}。运行单元验证脚本与主流程演示。上述步骤请在答辩前自行跑通，确保环境与文档一致。
