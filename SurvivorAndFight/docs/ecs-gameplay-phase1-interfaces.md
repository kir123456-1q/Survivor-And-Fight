...
- `pendingCast: { skillId: string; targetPos?: { x: number; y: number; z?: number } } | null` — 本帧待释放（由 ControlSystem 写入）
...
- 依赖：输入抽象接口（ControlInputSource：getMoveAxis、getSkillCastRequest）、FilterRegistry（Controllable）
...
### Targeting
- `resolveAuto(casterEntityId, candidates, world, attrSystem)` — 按威胁度与血量加权，优先低血高威胁
- `resolveSimple(casterEntityId, targetWorldPos, world)` — 以 targetWorldPos 为目标的简单索敌（最近）
