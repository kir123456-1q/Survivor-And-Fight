...
import { MetaMenuBootstrap } from './MetaMenuBootstrap';
...
export class SelectLevelPanelController extends UiControllerBase {
    ...
    private async handleLevelSelected(difficulty: RunDifficulty): Promise<void> {
        ...
        if (difficulty === TEST_LEVEL_DIFFICULTY) {
            MetaRunSession.resetRunRewards();
            MetaRunSession.resetTestSession();
            MetaRunSession.testMode = true;
            console.log('[UI] ui.meta.level.selected', JSON.stringify({
                difficulty,
                testMode: true,
                waves: 6,
                note: 'waves 4-6 auto ablation after wave 3',
            }));
            await this.uiStack.clearAll();
            MetaMenuBootstrap.hideSceneEmbeddedStartPanel();
            this.metaFlow.goto('Combat', {
                nodeId: 'test_level',
                payloadId: 'test_wave_1',
                testMode: true,
            });
            return;
        }
